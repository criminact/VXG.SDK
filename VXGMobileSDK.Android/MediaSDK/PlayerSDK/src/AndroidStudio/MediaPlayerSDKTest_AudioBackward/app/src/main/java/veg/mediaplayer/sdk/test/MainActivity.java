...
public class MainActivity extends Activity implements OnClickListener, MediaPlayer.MediaPlayerCallback
{
	    ...
	    public void handleMessage(Message msg) 
	    {
	        ...
	        switch (status) 
	        {
	        	...
	        	case PLP_BUILD_SUCCESSFUL:
					...
					Toast.makeText(getApplicationContext(),"Backward audio format="+sBackwardFormat, Toast.LENGTH_LONG).show();

					if(sBackwardFormat != null && sBackwardFormat.contains("PCMU")){
						btnMic.setEnabled(true);
						Toast.makeText(getApplicationContext(),"Backward audio channel is ready", Toast.LENGTH_LONG).show();
					}
	...
	private void verifyAudioRecordPermissions(Activity activity) {
		// Check if we have write permission
		...
		if (permission != PackageManager.PERMISSION_GRANTED) {
			...
			ActivityCompat.requestPermissions(
					this,  PERMISSIONS_RECORD_AUDIO,this.REQUEST_RECORD_AUDIO	);
		}
	}
    ...
    public void onCreate(Bundle savedInstanceState) 
	{
        ...
        btnMic = (Button) findViewById(R.id.button_mic);
		...
		//veg.mediacapture.sdk.MLog.isSignedWithDebugKey = true;
		...
		audioCapture.Open(null, new MediaCaptureCallback() {
			@Override
			...
			public int OnCaptureReceiveData(ByteBuffer buffer, int type, int size,
				...
				Log.v(TAG, "=OnCaptureReceiveData size="+size+" pts="+pts+" buffer.arrayOffset()="+buffer.arrayOffset());

				if(player.getState() == PlayerState.Started)
					player.audioPutBackwardAudio(buffer, size, pts);
	...
	RandomAccessFile fdump = null;
    void startBackwardAudio()
	{
		/*if(fdump==null){
			try {
				fdump = new RandomAccessFile(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DCIM)+"/test.raw", "rw");
		...
		if(ivLed != null)
			ivLed.setImageResource(R.drawable.led_red);
		btnMic.setText( "Stop MIC" );
    ...
    public void onClick(View v) 
	{
		...
		if (player != null)
		{
			...
			else
			{
    	    	...
    	    	//FOR BACKWARD AUDIO!!!
    	    	conf.setContentProviderLibrary(1);
    	    	conf.setEnableBackwardAudio(1);
    	    	//
